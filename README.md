...
![Version](https://img.shields.io/badge/version-1.3.0-26734d.svg)
...
V1.3.0 增加多租户 SaaS 运营和资源配额管理，当前仓库包含：
...
- 企业、部门、角色权限和审计管理页面。
- 员工档案新增、编辑、分页筛选、停用与恢复；
- 部门、职位、直属上级和登录账户关联；
- UTF-8 CSV 批量导入与逐行校验结果；
- 安全邀请链接、首次密码设置和待激活账户重新邀请；
- 企业级员工自助编辑范围和离职公开展示策略；
- 员工管理、个人资料与邀请激活页面。
- 企业默认名片模板、主题色、Logo、模块顺序和字段锁定；
- 员工名片基础资料、联系方式、社交账号和个性化配置；
- 草稿、已发布、已下线状态流转与独立发布快照；
- 发布前必填联系方式、图片地址和公开状态校验；
- 管理端桌面/手机实时预览和企业代管员工名片；
- 公开名片快照接口，未发布修改不会影响线上内容。
- 移动端优先的独立公开名片地址和不可用状态页；
- 本地生成的标准二维码、复制链接和带来源分享地址；
- 一键拨号、发送邮件、复制微信和下载 vCard 通讯录；
- 员工自主选择公开字段，公开接口只返回发布且允许展示的数据；
- 页面访问、来源渠道和关键按钮事件记录；
- 基于访客和时间窗口的访问去重及管理端名片统计。
- 企业产品分类及产品新增、编辑、排序、发布和下线；
- 产品封面、图集、规格、视频链接和 PDF 销售附件；
- 图片、视频和 PDF 企业素材库，以及类型、大小、访问范围和引用校验；
- 名片推荐产品草稿与发布快照，H5 产品列表和详情展示；
- 普通员工只读产品中心，产品与素材管理按独立权限点控制。
- 带隐私授权的公开名片咨询表单，可追溯名片、产品和分享来源；
- 相同联系方式 24 小时重复识别与明确提示；
- 名片归属人默认分配、企业管理员转交、销售领取和状态处理；
- 管理员企业范围与销售本人范围的线索数据隔离；
- 新线索和转交通知、未读数量及通知已读状态。
- 线索一键转为客户并完整保留名片、产品、来源和留资历史；
- 客户联系人、标签、负责人、归档及负责人数据权限；
- 文本跟进、跟进方式、发生时间和下次跟进提醒；
- 来源、留资、跟进、商机和管理操作统一客户时间线；
- 可配置商机阶段、预计金额、预计成交日期、阶段历史和简单漏斗；
- 客户合并冲突预览，以及联系人、跟进、商机和来源记录完整迁移。
- 企业初始化向导、明确空状态、统一错误页和关键危险操作确认；
- 健康检查、P95、错误率及创建—分享—留资—跟进发布指标；
...
- 平台套餐、试用订阅、到期续期记录和租户资源使用概览；
- 员工、名片和存储配额预警，以及超限后仅限制新增的存量保护；
- 企业冻结/恢复、注销确认与冷静期、数据导出和延迟清理策略；
- 有原因、有期限、有令牌且全程记录日志的平台临时协助授权。
...
| `INVITE_BASE_URL` | 否 | `http://localhost:5173/accept-invite` | 员工邀请激活页面地址 |
| `INVITE_EXPIRE_HOURS` | 否 | `72` | 邀请链接有效小时数，范围 1～168 |
| `PUBLIC_CARD_BASE_URL` | 否 | `http://localhost:5174/card` | 服务端生成公开名片及二维码地址的基础路径 |
| `VITE_PUBLIC_CARD_BASE_URL` | 否 | `http://localhost:5174/card` | 管理端复制公开名片链接的基础路径 |
...
```powershell
# 完整发布检查；本地服务已启动时会同时测试实时性能
.\scripts\release-check.ps1

# 创建带 SHA-256 清单的 SQLite 备份
.\scripts\backup.ps1

# 停止 API 后恢复指定备份；覆盖目标需明确传入 Force
.\scripts\restore.ps1 -Backup ".\data\backups\digitalcard-时间.db" -Force
```

详细流程见 [部署说明](docs/DEPLOYMENT.md)、[运维与恢复](docs/OPERATIONS.md) 和 [发布与缺陷响应](docs/RELEASE_AND_INCIDENTS.md)。
