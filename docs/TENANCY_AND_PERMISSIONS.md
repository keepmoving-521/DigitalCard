...
| 客户 | `customer.read`、`customer.self_manage`、`customer.all_manage` |
| 商机 | `opportunity.manage`、`opportunity.stage.manage` |
| 经营分析 | `analytics.read`、`analytics.all`、`analytics.export` |
| 营销 | `marketing.read`、`marketing.manage`、`marketing.export` |
| 业务预留 | `content.manage`、`customer.manage` |

后端权限依赖会在每次请求时读取当前企业角色绑定，因此角色权限更新无需用户重新登录。前端菜单与路由守卫用于改善体验，不能替代后端校验。

企业管理员默认拥有经营分析全部权限；内容管理员可查看企业范围并导出；销售和普通员工仅拥有 `analytics.read`，后端会将数据范围固定为当前员工。`analytics.all` 控制企业级查看范围，`analytics.export` 独立控制报表导出。
